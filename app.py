...
from data.loader import load_data, DEMO_DATASETS, validate_file_type, get_sample_files
from data.datasets import TensorDatasetWrapper, ImageDatasetWrapper, TextDatasetWrapper # Add Graph etc. later
from utils.visualization import plot_circuit, plot_state_vector, plot_training_history, plot_entanglement, plot_training_accuracy, plot_regression_predictions
...
if 'training_history' not in st.session_state:
    st.session_state.training_history = {'loss': [], 'entanglement': [], 'accuracy': []}
...
with tab_config:
    ...
    with col1:
        ...
        with st.expander("  Ansatz Entanglement Information"):
            st.markdown(f"### {ans_name}")
            st.markdown(f"**Description:** {ansatz_descriptions.get(ans_name, '')}")
            
            entanglement_ratings = {
                "Ansatz 1 (Rot+CNOT Chain)": "  (Medium)",
                "Hardware Efficient (All-to-All)": "  (High)",
                "GHZ-type (Star Topology)": "  (Very High)",
                "Brickwall (Alternating Pairs)": "  (High)",
                "Quantum Volume (Maximally Entangling)": "  (Very High)"
            }
            
            st.markdown(f"**Entanglement Rating:** {entanglement_ratings.get(ans_name, '')}")
            
            # Add link to documentation
            st.markdown("[📚 Learn more about entanglement-promoting ansatzes](docs/entanglement_ansatz.md)")
        ...
        if use_gpu and gpu_available and st.session_state.model_config.get('classical_backbone_type') == 'Transformer':
            st.warning("    Using GPU with Transformer models might cause issues in Streamlit. If you encounter errors, try disabling GPU or using a different backbone type.")
    ...
    with col2:
        ...
        if cb_type == "CNN":
            # Add safety check for CNN models
            default_cnn_model = st.session_state.model_config.get('classical_model_name', 'resnet18')
            if default_cnn_model not in AVAILABLE_CNN_MODELS:
                default_cnn_model = 'resnet18'
                
            cb_name = st.selectbox("CNN Model", AVAILABLE_CNN_MODELS, 
                                  index=AVAILABLE_CNN_MODELS.index(default_cnn_model), 
                                  key="classical_model_name_cnn", 
                                  help="Convolutional Neural Network model")

            if cb_name in ["resnet18", "resnet34", "resnet50"]:
                st.info(f"{cb_name} is a ResNet model. It will process input images and extract features.")
            elif cb_name == "mobilenet_v2":
                st.info("MobileNet V2 is designed for mobile and edge devices, offering a good balance between accuracy and speed.")
            with st.expander("CNN Architecture Details"):
                st.markdown(f"""
                **{cb_name}** is a pre-trained CNN model with the following characteristics:
                - Model Size: {{'resnet18': 'Small', 'resnet34': 'Medium', 'resnet50': 'Large', 'mobilenet_v2': 'Small-Medium'}}['{cb_name}']
                - Speed: {{'resnet18': 'Fast', 'resnet34': 'Medium', 'resnet50': 'Slower', 'mobilenet_v2': 'Very Fast'}}['{cb_name}']
                - Feature extraction quality: {{'resnet18': 'Good', 'resnet34': 'Better', 'resnet50': 'Best', 'mobilenet_v2': 'Good'}}['{cb_name}']
                """)

        elif cb_type == "Transformer":
            # Add safety check for Transformer models
            default_transformer_model = st.session_state.model_config.get('classical_model_name', 'bert-base-uncased')
            if default_transformer_model not in AVAILABLE_TRANSFORMER_MODELS:
                default_transformer_model = 'bert-base-uncased'
                
            cb_name = st.selectbox("Transformer Model", AVAILABLE_TRANSFORMER_MODELS, 
                                  index=AVAILABLE_TRANSFORMER_MODELS.index(default_transformer_model), 
                                  key="classical_model_name_transformer",
                                  help="Natural Language Processing model")
            ...
            if cb_name == "bert-base-uncased":
                st.info("BERT Base (Uncased) is a powerful language model for text understanding with 110M parameters.")
            elif cb_name == "distilbert-base-uncased":
                st.info("DistilBERT is a lighter and faster version of BERT with 66M parameters, retaining 97% of BERT's performance.")

        elif cb_type == "GNN":
            # Add safety check for GNN models
            default_gnn_model = st.session_state.model_config.get('classical_model_name', 'gcn')
            if default_gnn_model not in AVAILABLE_GNN_MODELS:
                default_gnn_model = 'gcn'
                
            cb_name = st.selectbox("GNN Model", AVAILABLE_GNN_MODELS, 
                                 index=AVAILABLE_GNN_MODELS.index(default_gnn_model), 
                                 key="classical_model_name_gnn")
            ...
            default_model = st.session_state.model_config.get('classical_model_name', 'linear')
        ...
        if st.button(" '  Create and Save Configuration", key="save_config_button"):
            config = {
                "num_qubits": nq,
                "num_layers": nl,
                "encoding_method": enc,
...
with tab_data:
    ...
    if not st.session_state.model_config:
        ...
        if data_source == "Upload":
            ...
            # Only offer demo datasets whose data actually exists on disk. The
            # housing dataset is always offered because it can be downloaded on
            # demand from the UI.
            relevant_demos = {}
            for name, info in DEMO_DATASETS.items():
                if info["type"] != data_type:
                    continue
                if name == "USA Housing Prices (Regression)" or os.path.exists(info["path"]):
                    relevant_demos[name] = info
            if relevant_demos:
        ...
        if data_type == "CSV":
            ...
            if raw_df is not None and isinstance(raw_df, pd.DataFrame):
                 ...
                 with col2:
                     ...
                     preprocess_button = st.button("⚙️ Preprocess Selected CSV Columns", key="preprocess_csv_button")
                 ...
                 if preprocess_button:
                     ...
                     if not selected_features:
                         ...
                         if st.session_state.model_config.get('classical_backbone_type') == 'Transformer':
                             st.warning("""
                                 You're using a Transformer model backbone, but preprocessing CSV columns directly.
        ...
        if st.button("📂 Load and Preprocess Data", key="load_data_button"):
            if data_source == "Upload" and not uploaded_file:
                ...
                with st.spinner("Loading and preprocessing..."):
                    ...
                    # NOTE: read from session state (not the local name) because the
                    # Cats-vs-Dogs / MNIST branches set session state directly and
                    # never bind a local `loaded_data`.
                    loaded_data = st.session_state.loaded_data
                    if (st.session_state.model_config.get('classical_backbone_type') == 'None'
                            and getattr(loaded_data, 'ndim', None) == 2):
                         expected_size = st.session_state.model_config['quantum_input_size']
                         if loaded_data.shape[-1] != expected_size:
        ...
        if (data_type == "CSV" and st.session_state.preview_data is not None and 
            ...
            if text_candidates and st.session_state.model_config.get('classical_backbone_type') == 'Transformer':
                st.info("📊 **CSV file contains text data and you selected a Transformer model.**")
                
                # Add a prominent message
                st.markdown("""
                <div style="padding:15px;border-radius:5px;background-color:#FFEB3B;color:#000;margin:15px 0;">
                    <h3 style="margin-top:0;">    Important: Direct Text Extraction Required for Transformer</h3>
                ...
                with tab1:
                    ...
                    with tab2:
                        ...
                        st.write("👉 Use the 'CSV Configuration' section below to select columns and preprocessing options.")
...
with tab_train:
    ...
    if not st.session_state.model_config or st.session_state.loaded_data is None:
        ...
        if (st.session_state.data_info.get('data_type') == 'CSV' and 
            ...
            st.warning("""
            ⚠️ **CSV data loaded but not fully preprocessed**
            ...
            3. Click "⚙️ Preprocess Selected CSV Columns"
        ...
        with col1:
            ...
            lr = st.number_input("Learning Rate", value=0.005, format="%.5f", key="learning_rate")
        ...
        with col2:
            ...
            if st.button("🔧 Instantiate Hybrid Model", key="instantiate_model_button"):
                    ...
                    try:
                        ...
                        if st.session_state.task_type == "Classification":
                            ...
                            if labels is None or len(labels) == 0:
                                st.error("Classification task selected, but no labels found. Load labeled data (e.g., CSV with Label Column).", icon="⚠️")
                                st.stop()
                            num_classes = len(torch.unique(labels)) if isinstance(labels, torch.Tensor) else len(labels.unique())
                            if num_classes < 2:
                                st.error(f"Classification task requires at least 2 classes, but found {num_classes} in labels.", icon="⚠️")
                                st.stop()
                            ...
                            if labels is None or len(labels) == 0:
                                st.error("Regression task selected, but no labels found. Load labeled data.", icon="⚠️")
                                st.stop()
            ...
            if st.session_state.hybrid_model is not None:
                ...
                if config_files:
                    ...
                    if load_weights_file != "None":
                        if st.button("📂 Load Weights", key="load_weights_button_train"):
        ...
        if st.session_state.hybrid_model is not None:
            ...
            if st.session_state.training_in_progress:
                st.warning("⚠️ **TRAINING IN PROGRESS** - Please wait for completion", icon="⚠️")
                ...
                training_indicator.markdown(
                    ...
                    <div style="padding:15px;border-radius:5px;background-color:#FFEB3B;color:#000;text-align:center;font-weight:bold;margin:10px 0;">
                        ⚙️ MODEL TRAINING ACTIVE - Please do not navigate away from this page
                    </div>
                    """, 
                    unsafe_allow_html=True
                )
            ...
            if st.button("🚀 Start Training", key="start_training_button", disabled=start_disabled):
            ...
            if st.session_state.training_in_progress:
                try:
                    ...
                    def log_progress(epoch, total_epochs, batch=None, total_batches=None, loss=None, elapsed_time=None, accuracy=None):
                        ...
                        if accuracy is not None:
                            progress_str += f" | Accuracy: {accuracy:.2%}"
                    ...
                    if task_type == "Classification":
                        if st.session_state.model_config.get('num_classes') is None:
                            st.error("Model not configured for Classification (num_classes missing). Instantiate model again.", icon="⚠️")
                            st.stop()
                        # Ensure labels are available
                        labels = st.session_state.get('csv_labels')
                        if labels is None:
                            st.error("Classification task requires labels, but none found in session state.", icon="⚠️")
                            st.stop()
                        ...
                        if not isinstance(labels, torch.Tensor):
                            try:
                                ...
                                st.error(f"Could not convert labels to required Long Tensor: {e}", icon="⚠️")
                                st.stop()
                        ...
                        if st.session_state.model_config.get('regression_output_size') is None:
                            st.error("Model not configured for Regression (regression_output_size missing). Instantiate model again.", icon="⚠️")
                            st.stop()
                        # Ensure labels are available
                        labels = st.session_state.get('csv_labels')
                        if labels is None:
                            st.error("Regression task requires labels, but none found in session state.", icon="⚠️")
                            st.stop()
                        ...
                        if not isinstance(labels, torch.Tensor):
                            try:
                                ...
                                st.error(f"Could not convert labels to required Float Tensor: {e}", icon="⚠️")
                                st.stop()
                        ...
                        # Create a custom loss function that applies proper scaling
                        def scaled_mse_loss(output, target):
                            # Use regular MSE for the calculation
                            mse = torch.nn.MSELoss()
                            loss = mse(output, target)
                            
                            # Apply scaling factor based on label magnitude to prevent extreme loss values
                            # This is especially important for housing prices which can be in millions
                            with torch.no_grad():
                                # Check if our output has the is_regression flag from the model
                                if hasattr(output, 'is_regression') and output.is_regression:
                                    # The preprocessing already takes care of scaling the labels
                                    # so we don't need additional scaling here
                                    pass
                                else:
                                    # Calculate scaling factor based on label magnitude
                                    # to keep loss values more stable during backpropagation
                                    target_scale = max(1.0, target.abs().mean().item()) 
                                    if target_scale > 1000:
                                        print(f"Applying scaling factor {1.0/target_scale:.6f} to regression loss")
                                        loss = loss / (target_scale * target_scale)
                            
                            return loss
                            
                        loss_fn = scaled_mse_loss
                        st.info("Using scaled MSELoss for Regression to improve stability.")
                        ...
                        if st.session_state.model_config['observation_type'] == 'State Vector':
                            ...
                            st.error("Unsupported observation type for unsupervised metric optimization.", icon="⚠️")
                            st.stop()
                    else:
                        st.error(f"Unknown task type selected: {task_type}", icon="⚠️")
                        st.stop()
                    ...
                    if data_type == "CSV" and backbone_type == 'cnn':
                        st.warning("""
                          **Warning:** You are using a CNN backbone with CSV data. 
                    ...
                    if data_type == "Images" and backbone_type == 'transformer':
                        st.warning("""
                          **Warning:** You are using a Transformer backbone with image data.
                    ...
                    if is_transformer:
                        ...
                        if not isinstance(input_data, dict) or 'input_ids' not in input_data or 'attention_mask' not in input_data:
                            ...
                            st.info("  Input data format is correctly tokenized for Transformer model.")
                    ...
                    live_chart_accuracy = st.empty()  # New chart for accuracy
                    ...
                    entanglement = np.nan
                    batch_entanglements = []
                    batch_accuracies = []  # New list to track batch accuracies
                    ...
                    accuracy_chart_data = pd.DataFrame({"Accuracy": [], "Step": []})  # New dataframe for accuracy
                    
                    # --- Materialize lazily-created layers and rebuild optimizer ---
                    # Hybrid models may create an input projection / output head on
                    # the first forward pass (arbitrary-size tabular input). Run a
                    # probe forward so those parameters exist before building the
                    # optimizer, otherwise they would never be trained.
                    try:
                        _probe = next(iter(data_loader))
                        _pf = _probe[0] if isinstance(_probe, (list, tuple)) else _probe
                        if isinstance(_pf, dict):
                            _pf = {k: v.to(st.session_state.device) for k, v in _pf.items()}
                        else:
                            _pf = _pf.to(st.session_state.device)
                        with torch.no_grad():
                            model(_pf)
                    ...
                    for epoch in range(epochs):
                        ...
                        for batch_idx, batch in enumerate(data_loader):
                            ...
                            try:
                                ...
                                batch_features = ensure_real(batch_features, target_dtype=torch.float32)
                                # Classification labels must stay integer (Long) for CrossEntropyLoss.
                                if batch_labels is not None and task_type != "Classification":
                            ...
                            if task_type in ["Classification", "Regression"]:
                                if batch_labels is None:
                                    st.error(f"{task_type} task requires labels, but none found in current batch.", icon="⚠️")
                                    st.stop()
                                ...
                                # Ensure labels are on the same device as output and have the
                                # dtype the loss expects (Long for classification).
                                ...
                                if task_type == "Classification":
                                    with torch.no_grad():
                                        ...
                                        # Calculate accuracy
                                        batch_accuracy = (preds == batch_labels).float().mean().item()
                            ...
                            if task_type == "Classification":
                                batch_status.text(f"Epoch {epoch+1}/{epochs} | Batch {batch_idx+1}/{total_batches} | Loss: {current_loss:.4f} | Accuracy: {batch_accuracies[-1]:.2%}")
                            else:
                                batch_status.text(f"Epoch {epoch+1}/{epochs} | Batch {batch_idx+1}/{total_batches} | Loss: {current_loss:.4f}")
                            
                            # Log progress every 10 batches
                            if batch_idx % 10 == 0:
                                if task_type == "Classification" and batch_accuracies:
                                    log_progress(
                                        epoch+1, epochs, 
                                        batch=batch_idx+1, 
                                        total_batches=total_batches, 
                                        loss=current_loss,
                                        accuracy=batch_accuracies[-1] if batch_accuracies else None
                                    )
                                else:
                                    log_progress(
                                        epoch+1, epochs, 
                            ...
                            # Add current accuracy to chart if available
                            if task_type == "Classification" and batch_accuracies:
                                new_acc_row = pd.DataFrame({"Accuracy": [batch_accuracies[-1]], "Step": [global_step]})
                                accuracy_chart_data = pd.concat([accuracy_chart_data, new_acc_row], ignore_index=True)
                                
                            # Redraw the charts with updated data
                            live_chart_loss.line_chart(loss_chart_data, x="Step", y="Loss")
                            if task_type == "Classification" and not accuracy_chart_data.empty:
                                live_chart_accuracy.line_chart(accuracy_chart_data, x="Step", y="Accuracy")
                            ...
                            if time_since_update > 1.0 or global_step % 5 == 0: 
                                ...
                                if task_type == "Classification" and batch_accuracies:
                                    status_text.text(f"Epoch {epoch+1}/{epochs} | Batch {batch_idx+1}/{total_batches} | Step {global_step} | Loss: {current_loss:.4f} | Accuracy: {batch_accuracies[-1]:.2%} | Entanglement (Q): {entanglement:.4f} | Elapsed: {elapsed_str}")
                                else:
                                ...
                                # Record batch details for run history
                                batch_detail = {
                                    "epoch": epoch + 1,
                                    "batch": batch_idx + 1,
                                    "step": global_step,
                                ...
                                if task_type == "Classification" and batch_accuracies:
                                    batch_detail["accuracy"] = batch_accuracies[-1]
                        ...
                        # Calculate average accuracy for the epoch if classification
                        if task_type == "Classification" and batch_accuracies:
                            avg_epoch_accuracy = np.mean(batch_accuracies)
                            st.session_state.training_history['accuracy'].append(avg_epoch_accuracy)
                            # Reset batch accuracies collection for next epoch
                            batch_accuracies = []
                        else:
                            # For non-classification, add placeholder
                            st.session_state.training_history['accuracy'].append(None)
                        ...
                        if task_type == "Classification" and 'accuracy' in st.session_state.training_history and st.session_state.training_history['accuracy'][-1] is not None:
                            status_text.text(f"Epoch {epoch+1}/{epochs} Completed | Avg Loss: {avg_epoch_loss:.4f} | Accuracy: {st.session_state.training_history['accuracy'][-1]:.2%} | Last Entanglement (Q): {entanglement:.4f} | Elapsed: {elapsed_str}")
                        ...
                        if task_type == "Classification" and 'accuracy' in st.session_state.training_history and st.session_state.training_history['accuracy'][-1] is not None:
                            log_progress(
                                epoch+1, epochs, 
                                loss=avg_epoch_loss,
                                accuracy=st.session_state.training_history['accuracy'][-1],
                                elapsed_time=elapsed_str
                            )
                        else:
                            log_progress(
                        ...
                        # Include accuracy in the chart if available
                        if task_type == "Classification" and 'accuracy' in st.session_state.training_history:
                            valid_accuracies = []
                            valid_epochs = []
                            for i, acc in enumerate(st.session_state.training_history['accuracy']):
                            ...
                            if valid_accuracies:
                                try:
                                    # Create visualization of training accuracy
                                    accuracy_img = plot_training_accuracy(valid_epochs, valid_accuracies)
                                    st.session_state.training_history['accuracy_chart'] = accuracy_img
                                except Exception as e:
                                    st.warning(f"Could not create accuracy visualization: {e}")
                            else:
                    ...
                    # Final accuracy if classification
                    if task_type == "Classification" and 'accuracy' in st.session_state.training_history:
                        final_accuracy = st.session_state.training_history['accuracy'][-1]
                    ...
                    run_details = {
                        ...
                        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                        "batch_info": batch_details[:100]  # Store up to 100 batch records to avoid too much data
                    }
                    
                    # If we have accuracy data, create a visualization
                    if task_type == "Classification" and 'accuracy' in st.session_state.training_history:
                        valid_accuracies = []
                        valid_epochs = []
                        for i, acc in enumerate(st.session_state.training_history['accuracy']):
                        ...
                        if valid_accuracies:
                            try:
                                # Create visualization of training accuracy
                                accuracy_img = plot_training_accuracy(valid_epochs, valid_accuracies)
                                run_details['accuracy_chart'] = accuracy_img
                            except Exception as e:
                                st.warning(f"Could not create accuracy visualization: {e}")
                    ...
                    if task_type == "Classification" and 'csv_labels' in st.session_state:
                        try:
                            ...
                            with torch.no_grad():
                                ...
                                # Store individual metric values in training_history for better visualization
                                if 'training_history' not in run_details:
                                    run_details['training_history'] = {}
                                
                                # Add accuracy to history if not already there
                                if 'accuracy' not in run_details['training_history'] and 'accuracy' in metrics:
                                    # Initialize with None values for epochs without accuracy data
                                    run_details['training_history']['accuracy'] = [None] * epochs
                                    # Add final accuracy as the last epoch value
                                    run_details['training_history']['accuracy'][-1] = float(metrics['accuracy'])
                                
                                # Add separate metric histories for better visualizations
                                for metric_name in ['precision', 'recall', 'f1', 'auc']:
                                    if metric_name in metrics and metrics[metric_name] is not None:
                                        # Store the final metric value
                                        if f"final_{metric_name}" not in run_details:
                                            run_details[f"final_{metric_name}"] = float(metrics[metric_name])
                                
                                # Include class names if available
                                if 'class_names' in st.session_state:
                                    run_details['class_names'] = st.session_state.class_names
                        ...
                        try:
                            ...
                            with torch.no_grad():
                                ...
                                # Store individual regression metric values
                                for metric_name in ['mae', 'mse', 'rmse', 'r2', 'rmsle']:
                                    if metric_name in metrics and metrics[metric_name] is not None:
                                        # Store the final metric value
                                        if f"final_{metric_name}" not in run_details:
                                            run_details[f"final_{metric_name}"] = float(metrics[metric_name])
                                
                                # Add regression prediction visualization if possible
                                try:
                                    if len(val_outputs) > 0 and len(labels) > 0:
                                        pred_fig = plot_regression_predictions(
                                            labels.cpu().numpy() if isinstance(labels, torch.Tensor) else labels,
                                            val_outputs.cpu().numpy() if isinstance(val_outputs, torch.Tensor) else val_outputs
                                        )
                                        # Store the figure in the run details
                                        if pred_fig is not None:
                                            from utils.run_history import plot_to_base64
                                            run_details['regression_plot'] = plot_to_base64(pred_fig)
                                except Exception as e:
                                    st.warning(f"Could not create regression plot: {e}")
...
with tab_predict:
    ...
    # Add special housing price prediction UI section if needed
    show_housing_form = True
    
    # Don't show housing form for Classification tasks when a model is loaded
    if st.session_state.hybrid_model and st.session_state.get('task_type', '') == 'Classification':
...
with tab_help:
    ...
    *   **Meyer-Wallach Measure (Q):** A measure of global entanglement in a multi-qubit system. Q=0 for a completely separable state, Q=1 for a maximally entangled state (averaged over bipartitions).
    ...
    1. **Ansatz 1 (Rot+CNOT Chain)** -   Medium Entanglement
       * Basic linear chain of CNOTs between adjacent qubits
       * Good default option for simple problems
    
    2. **Hardware Efficient (All-to-All)** -   High Entanglement
    ...
    3. **GHZ-type (Star Topology)** -   Very High Entanglement
    ...
    4. **Brickwall (Alternating Pairs)** -   High Entanglement
    ...
    5. **Quantum Volume (Maximally Entangling)** -   Very High Entanglement
