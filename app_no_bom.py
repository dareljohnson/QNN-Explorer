        ...
        if data_type == "CSV":
            ...
            if raw_df is not None and isinstance(raw_df, pd.DataFrame):
                 ...
                 with col2:
                     ...
                     preprocess_button = st.button("?????? Preprocess Selected CSV Columns", key="preprocess_csv_button")
...
with tab_train:
    ...
    if not st.session_state.model_config or st.session_state.loaded_data is None:
        ...
        if (st.session_state.data_info.get('data_type') == 'CSV' and 
            ...
            st.warning("""
            ?? ??? **CSV data loaded but not fully preprocessed**
            ...
            3. Click "?????? Preprocess Selected CSV Columns"
                    ...
                    try:
                        ...
                        if st.session_state.task_type == "Classification":
                            ...
                            if labels is None or len(labels) == 0:
                                st.error("Classification task selected, but no labels found. Load labeled data (e.g., CSV with Label Column).", icon="?? ???")
                                st.stop()
                            num_classes = len(torch.unique(labels)) if isinstance(labels, torch.Tensor) else len(labels.unique())
                            if num_classes < 2:
                                st.error(f"Classification task requires at least 2 classes, but found {num_classes} in labels.", icon="?? ???")
                                st.stop()
                            ...
                            if labels is None or len(labels) == 0:
                                st.error("Regression task selected, but no labels found. Load labeled data.", icon="?? ???")
                                st.stop()
        ...
        if st.session_state.hybrid_model is not None:
            ...
            if st.session_state.training_in_progress:
                st.warning("??"? **TRAINING IN PROGRESS** - Please wait for completion", icon="?? ???")
                ...
                training_indicator.markdown(
                    ...
                    <div style="padding:15px;border-radius:5px;background-color:#FFEB3B;color:#000;text-align:center;font-weight:bold;margin:10px 0;">
                        ??? MODEL TRAINING ACTIVE - Please do not navigate away from this page
                    </div>
                    """, 
                    unsafe_allow_html=True
                )
                ...
                try:
                    ...
                    if task_type == "Classification":
                        if st.session_state.model_config.get('num_classes') is None:
                            st.error("Model not configured for Classification (num_classes missing). Instantiate model again.", icon="???'")
                            st.stop()
                        # Ensure labels are available
                        labels = st.session_state.get('csv_labels')
                        if labels is None:
                            st.error("Classification task requires labels, but none found in session state.", icon="???'")
                            st.stop()
                        ...
                        if not isinstance(labels, torch.Tensor):
                            try:
                                ...
                                st.error(f"Could not convert labels to required Long Tensor: {e}", icon="???'")
                                st.stop()
                        ...
                        if st.session_state.model_config.get('regression_output_size') is None:
                            st.error("Model not configured for Regression (regression_output_size missing). Instantiate model again.", icon="???'")
                            st.stop()
                        # Ensure labels are available
                        labels = st.session_state.get('csv_labels')
                        if labels is None:
                            st.error("Regression task requires labels, but none found in session state.", icon="???'")
                            st.stop()
                        ...
                        if not isinstance(labels, torch.Tensor):
                            try:
                                ...
                                st.error(f"Could not convert labels to required Float Tensor: {e}", icon="???'")
                                st.stop()
                        ...
                        if st.session_state.model_config['observation_type'] == 'State Vector':
                            ...
                            st.error("Unsupported observation type for unsupervised metric optimization.", icon="???'")
                            st.stop()
                    else:
                        st.error(f"Unknown task type selected: {task_type}", icon="???'")
                        st.stop()
                        ...
                        for batch_idx, batch in enumerate(data_loader):
                            ...
                            if task_type in ["Classification", "Regression"]:
                                if batch_labels is None:
                                    st.error(f"{task_type} task requires labels, but none found in current batch.", icon="???'")
                                    st.stop()
